...
def train(model, model_loss, optimizer, TrainImgLoader, TestImgLoader, start_epoch, args):
    if (not is_distributed) or (dist.get_rank() == 0):
        ...
        # 定义绘图函数
        def plot_metrics(filename="final_metrics.png"):
            plt.figure(figsize=(12, 6))

            plt.subplot(1, 2, 1)
            plt.plot(range(len(train_loss_history)), train_loss_history, 'b-o', label='Train Loss')
            ...
            plt.subplot(1, 2, 2)
            if test_epochs:
                plt.plot(test_epochs, test_loss_history, 'r-o', label='Test Loss')
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        if (not is_distributed) or (dist.get_rank() == 0):
            ...
            train_loss_history.append(avg_train_metrics['loss'])
        ...
        if (epoch_idx % args.eval_freq == 0) or (epoch_idx == args.epochs - 1):
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                if (not is_distributed) or (dist.get_rank() == 0):
                    if do_summary:
                        ...
                        print(
                            "Epoch {}/{}, Iter {}/{}, test loss = {:.3f}, depth loss = {:.3f}, time = {:3f}".format(
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                ...
                test_epochs.append(epoch_idx)
                test_loss_history.append(test_metrics['loss'])
                save_scalars(tb_logger, 'fulltest', avg_test_scalars.mean(), global_step)
                print("avg_test_scalars:", avg_test_scalars.mean())
            gc.collect()

        if (not is_distributed) or (dist.get_rank() == 0) and (epoch_idx == args.epochs - 1):
            plot_metrics("final_metrics.png")
            with open("results/metrics.csv", "a") as f:
                f.write(f"{epoch_idx},{avg_train_metrics['loss']},{test_metrics['loss']},...\n")
